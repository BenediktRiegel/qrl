...
class FrozenLake4(Environment):
    ...
    def check_end_state_and_edges(
            self, edge_case: List[int], x_qubits: List[int], y_qubits: List[int],
            ancilla_qubits: List[int], unclean_qubits: List[int], oracle_qubit: int,
    ):
        # Allocate [oracle_qubit] + ancilla_qubits[:3] for edge qubits
        self.check_edges(x_qubits, y_qubits, [oracle_qubit] + ancilla_qubits[:3], ancilla_qubits[3:], unclean_qubits)
        for e_qubit, b in zip([oracle_qubit] + ancilla_qubits[:3], edge_case):
            if b == 0:
                qml.PauliX((e_qubit,))
        # Allocate ancilla_qubits[3] for e_qubit
        adaptive_ccnot([oracle_qubit] + ancilla_qubits[:3], ancilla_qubits[4:], x_qubits + y_qubits + unclean_qubits, ancilla_qubits[3])
        for e_qubit, b in zip([oracle_qubit] + ancilla_qubits[:3], edge_case):
            if b == 0:
                qml.PauliX((e_qubit,))
        # Deallocate [oracle_qubit] + ancilla_qubits[:3] for edge qubits
        self.check_edges(x_qubits, y_qubits, [oracle_qubit] + ancilla_qubits[:3], ancilla_qubits[4:], unclean_qubits + [ancilla_qubits[3]])

        # Allocate ancilla_qubits[0] for end_qubit
        self.check_end_state(x_qubits, y_qubits, [oracle_qubit] + ancilla_qubits[1:3] + ancilla_qubits[4:], unclean_qubits + [ancilla_qubits[3]], ancilla_qubits[0])
        # Allocate oracle_qubit for e_and_end_qubit
        adaptive_ccnot([ancilla_qubits[0]], ancilla_qubits[1:3] + ancilla_qubits[4:], x_qubits + y_qubits + unclean_qubits, oracle_qubit)
        # Deallocate ancilla_qubits[0] for end_qubit
        self.check_end_state(x_qubits, y_qubits, ancilla_qubits[1:3] + ancilla_qubits[4:], [oracle_qubit] + unclean_qubits + [ancilla_qubits[3]], ancilla_qubits[0])

        # Allocate ancilla_qubits[:3] + ancilla_qubits[4] for edge qubits
        self.check_edges(x_qubits, y_qubits, ancilla_qubits[:3] + [ancilla_qubits[4]], ancilla_qubits[5:], unclean_qubits)
        for e_qubit, b in zip(ancilla_qubits[:3] + [ancilla_qubits[4]], edge_case):
            if b == 0:
                qml.PauliX((e_qubit,))
        # Deallocate ancilla_qubits[3] for e_qubit
        adaptive_ccnot(ancilla_qubits[:3] + [ancilla_qubits[4]], ancilla_qubits[5:], x_qubits + y_qubits + unclean_qubits, ancilla_qubits[3])
        for e_qubit, b in zip(ancilla_qubits[:3] + [ancilla_qubits[4]], edge_case):
            if b == 0:
                qml.PauliX((e_qubit,))
        # Deallocate ancilla_qubits[:3] + ancilla_qubits[4] for edge qubits
        self.check_edges(x_qubits, y_qubits, ancilla_qubits[:3] + [ancilla_qubits[4]], ancilla_qubits[5:], unclean_qubits)
    ...
    def move_in_direction(
        ...
        edge_and_end_qubit = ancilla_qubits[0]
        ...
        for edge_case in self.possible_edge_cases:
            ...
            self.check_end_state_and_edges(edge_case, x_qubits, y_qubits, ancilla_qubits, next_x_qubits + next_y_qubits + unclean_qubits, edge_and_end_qubit)

            QAM(
                self.moves,
                [edge_and_end_qubit] + action_qubits,
            ...
            qml.adjoint(self.check_end_state_and_edges)(edge_case, x_qubits, y_qubits, ancilla_qubits, next_x_qubits + next_y_qubits + unclean_qubits, edge_and_end_qubit)
