...
class FrozenLake(Environment):
    def __init__(
            ...
            r_qubit_interpretation: List[str]
    ):
        super().__init__()
        self.r_qubit_interpretation = r_qubit_interpretation
        self.map = map
        # possible edge cases
        self.possible_edge_cases = self.get_possible_edge_cases()
        x_ceil_log2 = int(np.ceil(np.log2(len(map[0]))))
        self.x_add_bits = int_to_bitlist(2 ** x_ceil_log2 - len(map[0]), x_ceil_log2)
        y_ceil_log2 = int(np.ceil(np.log2(len(map))))
        self.y_add_bits = int_to_bitlist(2 ** y_ceil_log2 - len(map[0]), y_ceil_log2)
    ...
    def check_edges(self, x_qubits, y_qubits, edge_qubits, ancilla_qubits, unclean_qubits):
        # Edge on the right
        add_classical_quantum_registers(
            self.x_add_bits, x_qubits, ancilla_qubits, unclean_qubits, indicator_is_zero=True
        )
        adaptive_ccnot(x_qubits, ancilla_qubits, unclean_qubits, edge_qubits[0])
        qml.adjoint(add_classical_quantum_registers)(
            self.x_add_bits, x_qubits, ancilla_qubits, unclean_qubits, indicator_is_zero=True
        )
        ...
        # Edge above
        add_classical_quantum_registers(
            self.y_add_bits, y_qubits, ancilla_qubits, unclean_qubits, indicator_is_zero=True
        )
            ...
            for x_idx, field in enumerate(row):
                if field.end == 1:
                    simple_single_oracle(
                        x_qubits + y_qubits,
                        int_to_bitlist(x_idx, len(self.x_add_bits)) + int_to_bitlist(y_idx, len(self.y_add_bits)),
    ...
    def move_in_direction(
        ...
        end_state_qubit = ancilla_qubits[4]
        ancilla_qubits = ancilla_qubits[5:]

        # Check edges
        self.check_edges(x_qubits, y_qubits, edge_qubits, ancilla_qubits, unclean_qubits)
        ...
        self.check_end_state(x_qubits, y_qubits, ancilla_qubits, unclean_qubits, end_state_qubit)

        # Move
        for edge_case in self.possible_edge_cases:
            # Get modified slip amps
            modified_slip_amps = np.zeros(len(self.moves))
            for j, (b, prob, e_qubit) in enumerate(zip(edge_case, slip_probs, edge_qubits)):
                if b == 1:
            ...
            # Set edge qubits to 1
            for b, e_qubit in zip(edge_case, edge_qubits):
                if b == 0:
                    qml.PauliX((e_qubit,))
            QAM(self.moves, [end_state_qubit] + action_qubits + edge_qubits, next_x_qubits + next_y_qubits,
                ancilla_qubits, unclean_qubits, amplitudes=modified_slip_amps.tolist()
                ).circuit()

            # Undo set edge qubits to 1
            for b, e_qubit in zip(edge_case, edge_qubits):
                if b == 0:
                    qml.PauliX((e_qubit,))
        ...
        # Undo edge check
        qml.adjoint(self.check_edges)(x_qubits, y_qubits, edge_qubits, ancilla_qubits, unclean_qubits)
    ...
    def move(
        ...
        # Add state
        add_registers(x_qubits, next_x_qubits, ancilla_qubits, unclean_qubits, indicator_is_zero=True)
        add_registers(y_qubits, next_y_qubits, ancilla_qubits, unclean_qubits, indicator_is_zero=True)

    def compute_rewards(
            self, x_qubits: List[int], y_qubits: List[int], r_qubits: List[int],
            ancilla_qubits: List[int], unclean_qubits: List[int] = None
    ):
        ...
        for x_idx, row in enumerate(self.map):
            for y_idx, field in enumerate(row):
                if field.reward != 0:
                    simple_single_oracle(x_qubits + y_qubits,
                                         int_to_bitlist(x_idx, len(self.x_add_bits)) + int_to_bitlist(y_idx,
                                                                                                 len(self.y_add_bits)),
                                         ancilla_qubits[1:], unclean_qubits, oracle_qubit)
                    for r_qubit, exp in zip(r_qubits, self.r_qubit_interpretation):
                        if get_bit_by_interpretation(field.reward, exp) == 1:
                            qml.CNOT((oracle_qubit, r_qubit))
                    simple_single_oracle(x_qubits + y_qubits,
                                         int_to_bitlist(x_idx, len(self.x_add_bits)) + int_to_bitlist(y_idx,
                                                                                                 len(self.y_add_bits)),
                                         ancilla_qubits[1:], unclean_qubits, oracle_qubit)
    ...
    def circuit(
        ...
        """
        # Move
        self.move(x_qubits, y_qubits, action_qubits, next_x_qubits, next_y_qubits, ancilla_qubits, unclean_qubits)

        # Rewards
        self.compute_rewards(next_x_qubits, next_y_qubits, r_qubits, ancilla_qubits, unclean_qubits)


class FrozenLake2(Environment):
    def __init__(
    ...
    def move_in_direction(
        ...
        # Check edges
        self.check_edges(
            x_qubits, y_qubits, edge_qubits,
            next_x_qubits + next_y_qubits + [end_state_qubit] + ancilla_qubits,
            action_qubits + unclean_qubits,
        )
        ...
        self.check_end_state(
            ...
            action_qubits + edge_qubits + unclean_qubits,
            end_state_qubit,
        )
        ...
        for edge_case in self.possible_edge_cases:
            ...
            QAM(
                self.moves,
                [end_state_qubit] + action_qubits + edge_qubits,
        ...
        self.check_end_state(
            x_qubits, y_qubits,
            ancilla_qubits,
            next_x_qubits + next_y_qubits + action_qubits + edge_qubits + unclean_qubits,
            end_state_qubit,
        )
