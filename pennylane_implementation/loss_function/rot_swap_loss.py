...
def value_loss(
    ...
    if snaps:
        ...
        for i in range(len(state) // num_interesting):
            ...
            if p != 0:
                if np.count_nonzero(s) != 0:
                    ...
                    print(f"p: {p}")
            s = s[::swap_state_size]
            sum += p * np.square(np.abs(np.sum(s)))
        ...
        print(f"precise result: {true_prob[0] - result[1]}")
...
def action_loss(
    ...
    return (result[1] - result[0]) * vector_norm * 2 * v_max
...
def loss_function(
    ...
    loss2 = value_loss(
        action_qnn, value_qnn, environment, x_qubits, y_qubits, action_qubits, next_x_qubits, next_y_qubits,
        ancilla_qubits[:3], ancilla_qubits[3], ancilla_qubits[4:7], ancilla_qubits[7], ancilla_qubits[8:],
