...
from qnns import QNN, RotQNN, CCRotQNN, CCRotQNN2
...
def main():
    ...
    # Loss function2
    wires, total_num_wires = get_wires([log_cols, log_rows, 2, log_cols, log_rows, len(r_qubit_interpretation), len(r_qubit_interpretation)+3, len(r_qubit_interpretation)+3, 2, 2])
...
def rot_main():
    ...
    r_qubit, value_qubit, next_value_qubit, loss1_qubit, loss2_qubit = r_qubit[0], value_qubit[0], next_value_qubit[0], loss1_qubit[0], loss2_qubit[0]
    backend = backend.get_pennylane_backend("", "", total_num_wires, shots)
    print(f"{total_num_wires} qubits")
    ...
    frames, losses = train(loss_fn, optimizer, num_iterations, sub_iterations, action_qnn, value_qnn, loss_function_params)
