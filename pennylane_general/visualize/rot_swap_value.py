...
def get_value(x, y, value_qnn, num_x_qubits, num_y_qubits, v_m):
    wires, total_num_wires = get_wires([num_x_qubits, num_y_qubits, num_x_qubits, num_y_qubits, 2, 1, 3, 2])
    x_qubits, y_qubits, \
        load_x_qubits, load_y_qubits, \
        value_indices_qubits, value_qubit, \
        swap_vector_qubits, \
        ancilla_qubits = wires

    value_qubit = value_qubit[0]
    backend = QuantumBackends.pennylane_default_qubit.get_pennylane_backend("", "", total_num_wires, None)
    ...
    def circuit():
        for (x_q, l_x_q, x_b) in zip(x_qubits, load_x_qubits, x_bits):
            if x_b == 1:
                qml.PauliX((x_q,))
                qml.PauliX((l_x_q,))
        for (y_q, l_y_q, y_b) in zip(y_qubits, load_y_qubits, y_bits):
            if y_b == 1:
                qml.PauliX((y_q,))
                qml.PauliX((l_y_q,))

        indices_vector = np.array([[1, 1, 1, 0]], dtype=np.float64)
        indices_vector /= np.linalg.norm(indices_vector[0])

        LittleTreeLoader(
            indices_vector, value_indices_qubits,
            ancilla_wires=ancilla_qubits,
            unclean_wires=x_qubits + y_qubits
        ).circuit()

        value_qnn.circuit(
            load_x_qubits + load_y_qubits, x_qubits + y_qubits, value_qubit,
            control_qubits=[value_indices_qubits[0]],
            ancilla_qubits=ancilla_qubits, unclean_qubits=[],
        )
        value_qnn.circuit(
            load_x_qubits + load_y_qubits, x_qubits + y_qubits, value_qubit,
            control_qubits=[value_indices_qubits[1]],
            ancilla_qubits=ancilla_qubits, unclean_qubits=[],
            conjugate=True,
        )

        swap_vector = np.array([[2, -1, -1, 0]], dtype=np.float64)
        swap_vector /= np.linalg.norm(swap_vector[0])

        for q in swap_vector_qubits[:-3]:
            qml.Hadamard((q,))

        LittleTreeLoader(
            swap_vector, swap_vector_qubits[-3:-1],
            ancilla_wires=ancilla_qubits,
            unclean_wires=x_qubits + y_qubits
        ).circuit()

        swap_test(ancilla_qubits[0], value_indices_qubits + [value_qubit], swap_vector_qubits)


        return qml.expval(qml.PauliZ(wires=(ancilla_qubits[0],)))

    result = qml.QNode(circuit, backend)()
