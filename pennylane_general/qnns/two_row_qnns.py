...
class CCRYQNN_Excessive:
    ...
    def variational_layer(
            self, layer_num: int,
            control_qubits: List[int],
            compute_qubits: List[int], result_qubit: int,
            ancilla_qubits: List[int] = None, unclean_qubits: List[int] = None,
            conjugate: bool = False,
    ):
        for idx, q_param in enumerate(self.in_q_parameters[layer_num]):
            bits = int_to_bitlist(idx, self.num_input_qubits)
            cc_simple_single_oracle(control_qubits, compute_qubits, bits, ancilla_qubits[1:], unclean_qubits,
                                    ancilla_qubits[0])
    ...
    def load_layer(
            ...
            compute_qubits: List[int],
            ancilla_qubits: List[int] = None, unclean_qubits: List[int] = None,
            conjugate: bool = False,
    ):
        for in_q, com_q in zip(input_qubits, compute_qubits):
            adaptive_ccnot(control_qubits + [in_q], ancilla_qubits, unclean_qubits, com_q)

    def layer(
            self, layer_num: int,
            control_qubits: List[int],
            input_qubits: List[int], compute_qubits: List[int], result_qubit: int,
            ancilla_qubits: List[int] = None, unclean_qubits: List[int] = None,
            conjugate: bool = False,
    ):
        if layer_num != 0:
            self.load_layer(layer_num, control_qubits, input_qubits, compute_qubits, ancilla_qubits, unclean_qubits, conjugate)
        self.variational_layer(layer_num, control_qubits, compute_qubits, result_qubit, ancilla_qubits, unclean_qubits)

    def circuit(
            self,
            input_qubits: List[int], compution_qubits: List[int], result_qubit: int,
            control_qubits: List[int] = None, ancilla_qubits: List[int] = None, unclean_qubits: List[int] = None,
            conjugate: bool = False,
    ):
        control_qubits = [] if control_qubits is None else control_qubits
        ...
        for d in range(self.depth):
            self.layer(d, control_qubits, input_qubits, compution_qubits, result_qubit, ancilla_qubits, unclean_qubits, conjugate)
    ...
    def get_circuit(
        ...
        def circuit():
            return self.circuit(input_qubits, compution_qubits, result_qubit, control_qubits, ancilla_qubits, unclean_qubits)
...
class RYQNN_Excessive:
    def __init__(
        ...
        if weight_init == WeightInitEnum.standard_normal:
            ...
            self.in_q_parameters = torch.nn.Parameter(
                torch.zeros((depth, 2 ** self.num_input_qubits, num_output_qubits)),
                requires_grad=True)
        else:
            raise NotImplementedError("Unknown weight init method")
    ...
    def variational_layer(
            self, layer_num: int,
            compute_qubits: List[int], output_qubits: List[int],
            ancilla_qubits: List[int] = None, unclean_qubits: List[int] = None,
            conjugate: bool = False,
    ):
        for idx, q_out_params in enumerate(self.in_q_parameters[layer_num]):
            bits = int_to_bitlist(idx, self.num_input_qubits)
            simple_single_oracle(compute_qubits, bits, ancilla_qubits[1:], unclean_qubits, ancilla_qubits[0])
    ...
    def load_layer(
        ...
        for in_q, com_q in zip(input_qubits, compute_qubits):
            qml.CNOT(wires=(in_q, com_q))

    def layer(
            self, layer_num: int,
            input_qubits: List[int], compute_qubits: List[int], output_qubits: List[int],
            ancilla_qubits: List[int] = None, unclean_qubits: List[int] = None,
            conjugate: bool = False,
    ):
        if layer_num != 0:
            self.load_layer(layer_num, input_qubits, compute_qubits, ancilla_qubits, unclean_qubits, conjugate)
        self.variational_layer(layer_num, compute_qubits, output_qubits, ancilla_qubits, unclean_qubits, conjugate)
    ...
    def circuit(
        ...
        unclean_qubits = [] if unclean_qubits is None else unclean_qubits
        for d in range(self.depth):
            self.layer(d, input_qubits, compute_qubits, output_qubits, ancilla_qubits, unclean_qubits, conjugate)
    ...
    def get_circuit(
        ...
        def circuit():
            return self.circuit(input_qubits, compute_qubits, output_qubits, ancilla_qubits, unclean_qubits, conjugate)
...
class CCRYQNN_One:
    def __init__(
        ...
        # weight init
        if weight_init == WeightInitEnum.standard_normal:
            self.in_q_parameters = [[torch.nn.Parameter(el + add_value, requires_grad=False) for el in layer_p] for
                                    layer_p in torch.pi * torch.randn((depth, 2 ** num_input_qubits))]
        elif weight_init == WeightInitEnum.uniform:
            self.in_q_parameters = [[torch.nn.Parameter(el + add_value, requires_grad=False) for el in layer_p] for
                                    layer_p in torch.pi * torch.rand((depth, 2 ** num_input_qubits))]
        elif weight_init == WeightInitEnum.zero:
            self.in_q_parameters = [[torch.nn.Parameter(el + add_value, requires_grad=False) for el in layer_p] for
                                    layer_p in torch.pi * torch.zeros((depth, 2 ** num_input_qubits))]
        else:
            raise NotImplementedError("Unknown weight init method")
    ...
    def layer(
        ...
        if layer_num != 0:
            self.load_layer(layer_num, control_qubits, input_qubits, compute_qubits, ancilla_qubits, unclean_qubits,
                            conjugate)
        self.variational_layer(layer_num, control_qubits, compute_qubits, result_qubit, ancilla_qubits, unclean_qubits,
                               conjugate)
    ...
    def circuit(
        ...
        unclean_qubits = [] if unclean_qubits is None else unclean_qubits
        for d in range(self.depth):
            self.layer(d, control_qubits, input_qubits, compute_qubits, result_qubit, ancilla_qubits, unclean_qubits,
                       conjugate)
    ...
    def get_circuit(
        ...
        def circuit():
            return self.circuit(input_qubits, compute_qubits, result_qubit, control_qubits, ancilla_qubits,
                                unclean_qubits, conjugate)
